...
class new_thread(threading.Thread):
    def __init__(self, file_paths, tID):
        threading.Thread.__init__(self)
        self.file_paths = file_paths
        self.threadID = tID

    def run(self):
        figure = plot.figure("Crash Labeller - Thread " + str(self.threadID))
        ...
        for path in files:
            ...
            point = self.find_bottom_edge(img_bw)
    ...
    def find_bottom_edge(self, img_bw):
...
for i in xrange(threads):
    threads_list.append(new_thread(files[len(files) / threads * i:len(files) / threads * (i + 1) - 1], i))

for delegate in threads_list:
    delegate.start()
